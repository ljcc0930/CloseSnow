...
## Request ID
- `coordinate-fixes-20-heavenly-ca`

## Title
- Correct Heavenly catalog coordinates
...
## Background
- GitHub issue #20 reports that `https://ljcc0930.github.io/CloseSnow/resort/heavenly-ca/` currently shows `38.000000, -120.500000`, which is not the intended Heavenly, CA resort location.
...
- This request is intentionally scoped as a data correction for the `heavenly-ca` catalog entry, not as a new coordinate-handling feature.

## Goal
- Add a verified explicit coordinate override for the `heavenly-ca` entry in `resorts.yml` so CloseSnow resolves Heavenly, CA to the corrected resort location instead of the issue-reported point `38.000000, -120.500000`.
- Keep the fix isolated to this resort so the merged PR can be traced directly back to GitHub issue #20.
...
## Acceptance Criteria
- [ ] The `resorts.yml` entry for `heavenly-ca` includes explicit `latitude` and `longitude` values that map to a verified Heavenly, CA resort location rather than `38.000000, -120.500000`.
...
## Test Plan
- `python3 -c 'from src.backend.resort_catalog import load_resort_catalog; entry = next(x for x in load_resort_catalog("resorts.yml") if x["resort_id"] == "heavenly-ca"); assert entry["latitude"] is not None and entry["longitude"] is not None; print(entry["latitude"], entry["longitude"])'`
- `python3 -m pytest -q tests/backend/test_resort_catalog.py tests/backend/test_weather_data_server_hourly.py`
- `python3 -m src.cli static --output-dir /tmp/coordinate-fixes-20-heavenly-ca --max-workers 8`
- `python3 -c 'import json; payload = json.load(open("/tmp/coordinate-fixes-20-heavenly-ca/resort/heavenly-ca/hourly.json")); assert (payload.get("resolved_latitude"), payload.get("resolved_longitude")) != tuple(float(x.strip()) for x in "38.000000, -120.500000".split(",")); print(payload.get("resolved_latitude"), payload.get("resolved_longitude"))'`
