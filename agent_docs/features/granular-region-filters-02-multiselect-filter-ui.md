...
## Title
- Replace homepage filter dropdowns with multiselect controls
...
- The requested UX needs a filter panel that follows the reference image more closely: finer region buckets and multiselect controls instead of dropdowns.

## Goal
- Rework the homepage filter modal so `Region` uses the new image-aligned subregions as a checkbox-based multiselect, `Country` also becomes a checkbox-based multiselect, and `Sort By` becomes a non-dropdown single-choice control while preserving immediate filter application, localStorage persistence, and dynamic API refetch behavior.
...
- Do not leave remaining region or country dropdown selects in the modal. Sort can stay single-choice, but it should render as radios or buttons instead of a `<select>`.
...
- [ ] Sort is rendered without a dropdown and still allows exactly one active choice.
