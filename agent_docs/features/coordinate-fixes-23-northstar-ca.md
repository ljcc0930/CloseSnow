...
## Request ID
- `coordinate-fixes-23-northstar-ca`

## Title
- Correct Northstar catalog coordinates
...
## Background
- GitHub issue #23 reports that `https://ljcc0930.github.io/CloseSnow/resort/northstar-ca/` currently shows `39.250000, -120.250000`, which is not the intended Northstar, CA resort location.
...
- This request is intentionally scoped as a data correction for the `northstar-ca` catalog entry, not as a new coordinate-handling feature.

## Goal
- Add a verified explicit coordinate override for the `northstar-ca` entry in `resorts.yml` so CloseSnow resolves Northstar, CA to the corrected resort location instead of the issue-reported point `39.250000, -120.250000`.
- Keep the fix isolated to this resort so the merged PR can be traced directly back to GitHub issue #23.
...
## Acceptance Criteria
- [ ] The `resorts.yml` entry for `northstar-ca` includes explicit `latitude` and `longitude` values that map to a verified Northstar, CA resort location rather than `39.250000, -120.250000`.
...
## Test Plan
- `python3 -c 'from src.backend.resort_catalog import load_resort_catalog; entry = next(x for x in load_resort_catalog("resorts.yml") if x["resort_id"] == "northstar-ca"); assert entry["latitude"] is not None and entry["longitude"] is not None; print(entry["latitude"], entry["longitude"])'`
- `python3 -m pytest -q tests/backend/test_resort_catalog.py tests/backend/test_weather_data_server_hourly.py`
- `python3 -m src.cli static --output-dir /tmp/coordinate-fixes-23-northstar-ca --max-workers 8`
- `python3 -c 'import json; payload = json.load(open("/tmp/coordinate-fixes-23-northstar-ca/resort/northstar-ca/hourly.json")); assert (payload.get("resolved_latitude"), payload.get("resolved_longitude")) != tuple(float(x.strip()) for x in "39.250000, -120.250000".split(",")); print(payload.get("resolved_latitude"), payload.get("resolved_longitude"))'`
