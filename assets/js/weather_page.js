...
const filterSortInputs = Array.from(document.querySelectorAll("input[name='filter-sort-by']"));
...
const applyControlsFromQueryOrMeta = () => {
  ...
  filterSortInputs.forEach((input) => {
    input.checked = input.value === sortBy;
  });
...
const applyFilterStateFromControls = () => {
  ...
  const selectedCountries = filterCountryOptions
    ? Array.from(filterCountryOptions.querySelectorAll("input[name='filter-country']:checked"))
      ...
      .filter((value) => /^[A-Z]{2}$/.test(value))
    : [];
  const selectedSort = filterSortInputs.find((input) => input.checked);
  appState.filterState.subregions = new Set(selectedSubregions);
  appState.filterState.countries = new Set(selectedCountries);
  appState.filterState.sortBy = normalizeSortBy(selectedSort ? selectedSort.value : "week_snow");
...
const resetFilterControls = () => {
  ...
  filterSortInputs.forEach((input) => { input.checked = input.value === "week_snow"; });
